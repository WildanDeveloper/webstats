...
This setup serves the dashboard on your domain with automatic SSL (Let's
...
```
Internet ──> Caddy :80/:443 ──> stats.yourdomain.com   ──> Next.js :3000
                              ├─> api.yourdomain.com    ──> dashboard API :8086
                              └─> tracker.yourdomain.com ──> ingest API :8085
```

## 1. DNS

Point these A records at your server IP:

| Host                   | Type | Value     |
|------------------------|------|-----------|
| `stats.yourdomain.com` | A    | server IP |
| `api.yourdomain.com`   | A    | server IP |
| `tracker.yourdomain.com` | A  | server IP |
...
sudo nano /etc/webstats.env     # fill JWT_SECRET, IP_HASH_SALT, domains
```
...
curl -s https://tracker.yourdomain.com/track.js | head -c 80
```
...
- **Tracker data not arriving**: check the site install code uses `tracker.yourdomain.com/track.js`, and `ALLOW_ORIGINS` includes `https://stats.yourdomain.com`.
