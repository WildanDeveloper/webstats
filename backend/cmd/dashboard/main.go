...
func main() {
	...
	api.Get("/ping/:key", heartbeatPingHandler(pool))
	api.Post("/ping/:key", heartbeatPingHandler(pool))

	pub := api.Group("/public")
	pub.Get("/:token/overview", publicOverviewHandler(pool))
	pub.Get("/:token/timeseries", publicTimeseriesHandler(pool))
	pub.Get("/:token/sessions", publicSessionStatsHandler(pool))
	pub.Get("/:token/entry-pages", publicSessionBoundsHandler(pool, "entry"))
	pub.Get("/:token/exit-pages", publicSessionBoundsHandler(pool, "exit"))
	...
	stats.Get("/sessions", sessionStatsHandler(pool))
	stats.Get("/entry-pages", sessionBoundsHandler(pool, "entry"))
	stats.Get("/exit-pages", sessionBoundsHandler(pool, "exit"))
	...
	stats.Get("/vitals", vitalsHandler(pool))
	stats.Get("/heartbeats", listHeartbeatsHandler(pool, cfg))
	stats.Post("/heartbeats", createHeartbeatHandler(pool, cfg))
	stats.Delete("/heartbeats/:hid", deleteHeartbeatHandler(pool))
	stats.Get("/maintenance", listMaintenanceHandler(pool))
	stats.Post("/maintenance", createMaintenanceHandler(pool))
	stats.Delete("/maintenance/:wid", deleteMaintenanceHandler(pool))
	stats.Get("/incidents", incidentsHandler(pool))
