          ...
          <tbody>
            {rows.map((r, i) => (
              <tr
                ...
                className={`border-b border-edge last:border-0 ${onSelect ? "cursor-pointer" : ""}`}
              >
                ...
                <td className="w-full py-2.5 pr-4">
                  <div className="flex items-center gap-3">
                    <span className={`max-w-[240px] truncate ${onSelect ? "text-soft hover:text-indigo-400" : "text-soft"}`}>
                      {r.key}
                    </span>
        ...
        <ul className="space-y-2.5">
          {rows.map((r) => (
            <li
              key={r.key}
              onClick={() => onSelect?.(r.key)}
              className={`flex items-center gap-3 text-sm ${onSelect ? "cursor-pointer" : ""}`}
            >
              <span className={`max-w-[180px] truncate ${onSelect ? "text-soft hover:text-indigo-400" : "text-soft"}`}>{r.key}</span>
